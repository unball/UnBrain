...
from MainVision.controller.strategy.movements import goToBall, goToGoal, projectBall, howFrontBall, howPerpBall, goalkeep, blockBallElipse, mirrorPosition, spinDefender, spinGoalKeeper
from MainVision.controller.strategy.field import UVFDefault, GoalKeeperField, DefenderField, UVFavoidGoalArea
from MainVision.controller.tools import ang, angError, norm, unit, projectLine
import numpy as np
import time
...
class Defender(Entity):
    ...
    def movementDecider(self):
        ...
        vb = np.array(self.world.ball.vel.copy())
        rr = np.array(self.robot.pose)
        rg = np.array(self.world.allyGoalPos)
...
class GoalKeeper(Entity):
    ...
    def movementDecider(self):
        ...
        vb = np.array(self.world.ball.vel.copy())
        rr = np.array(self.robot.pose)
        rg = (np.array(self.world.allyGoalPos) + [self.robot.size / 2, 0])
