...
from MainVision.view.tools.frameSelector import FrameRenderer
from MainVision.view.tools.drawing import Drawing
from MainVision.controller.tools import norm
from MainVision.controller.tools.pixel2metric import normToAbs
from MainVision.model.vision.cortarCampoInternoModel import CortarCampoInternoModel
import cv2
import numpy as np
...
class CortarCampoInterno(FrameRenderer):
  ...
  def getFrame(self):
    ...
    # Preenche o polígono
    if self.__model.done is True:
      mask = self.__visionSystem.get_polygon_mask(frame, points)
