...
class SystemTester:
    ...
    def gera_datadict(self, register_dict : dict) -> dict: 
        ...
        goal_register = register_dict["goalcount"]
        ...
        goal_intervals = [interval 
                          for thread_intervals in goal_register
                            for interval in thread_intervals]
        goal_intervals_mean = sum(goal_intervals)/len(goal_intervals) if len(goal_intervals) != 0 else 0
        quant_goals = len(goal_intervals)
        ...
        data_dict = {"runningtime": formatted_runningtime,
                                 "goalcount": quant_goals,
                                 "goalinterval_mean": goal_intervals_mean,
