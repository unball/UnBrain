...
DEBUG_ACTUATE = False  #Variable that verifies if the actuate is sending the methods (use it for when there's no transmiter),
...
def port_fira(team_yellow, is_travesim=False):
    if is_travesim:
        if team_yellow:
            return 20012
        else:
