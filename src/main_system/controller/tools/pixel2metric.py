import math
from main_system.controller.tools import unit, angl

def _safe_int(v):
  """Converte para int tratando NaN/inf como 0, para nunca quebrar o desenho.
  Dados do estado podem trazer NaN (ex.: velocidade não inicializada); um artefato
  visual em (0,0)/tamanho 0 é preferível a derrubar a thread do renderizador."""
...
def meters2pixel(world, position, shape):
  ...
  return (_safe_int(x),_safe_int(y))
