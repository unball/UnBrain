from main_system.controller.tools import angError, shift, angl, unit, adjustAngle
import numpy as np
...
class Element(object):
  ...
  def raw_pose(self):
    """Retorna a pose \\([x,y,\\theta]\\) do objeto um numpy array."""
    return np.array([self._raw_x, self._raw_y, self._raw_th])
  ...
  def calc_velocities(self, dt, alpha=0.5, thalpha=0.8, accalpha=0.2):
    ...
    # O filtro de Kalman na Visão Computacional deve estar sempre desligado,
    # pois o UnBrain já possui a sua própria filtragem!
    if True: # not getattr(self.world, 'flag_use_kalman', False):
        ...
        # Codigo do Kalman foi ignorado propositalmente
        pass
    ...
    if True: # not getattr(self.world, 'flag_use_kalman', False):
        self.inst_w = (angError(self._raw_th, self._prev_raw_th)/dt)*thalpha + (self.inst_w)*(1-thalpha)
