...
class CameraHandlerView():
  ...
  def __init__(self, controller, cameraHandler, menuButton):
    ...
    self.__pause_video = builder.get_object("pausar_video")
    ...
    # Adiciona a câmera "padrão" que é a imagem padrão e o vídeo
    self.add_to_list_box(self.__cameraPopoverCamList, -1, "Padrão")
    self.add_to_list_box(self.__cameraPopoverCamList, -2, "Vídeo de Teste")
    ...
    if self.__pause_video:
        self.__pause_video.connect("state-set", self.update_pause)
  ...
  def update_pause(self, widget, value):
    """Pausa ou despausa a coleta de frames do OpenCV no CameraHandler"""
    self.__cameraHandler.set_paused(value)
