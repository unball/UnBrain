...
from main_system.model.vision.cortarCampoModel import CortarCampoModel
import cv2
import numpy as np
...
class CortarCampo(FrameRenderer):
  ...
  def getFrame(self):
    ...
    if self.__show_warpped: return self.__visionSystem.warp(frame)
      ...
      # Pega uma cópia da lista para thread safety
      pts = list(self.__model.clicked_points_homography)
      color = (0,255,0) if len(pts) == 4 else (255,255,255)
      mousePos = normToAbs(self.__current_mouse_position, frame.shape)
  
      # Draw line for each pair of points
      if len(pts) > 1:
        for i in range(len(pts)-1):
          p0 = normToAbs(pts[i], frame.shape)
          p1 = normToAbs(pts[i+1], frame.shape)
          cv2.line(frame, p0, p1, color, thickness=2)
      
      # Closes rectangle
      if len(pts) == 4:
        p0 = normToAbs(pts[0], frame.shape)
        p1 = normToAbs(pts[3], frame.shape)
        cv2.line(frame, p0, p1, color, thickness=2)
      
      # Draw helping line from last chosen point to current mouse position
      if len(pts) > 0 and len(pts) < 4:
        plast = normToAbs(pts[-1], frame.shape)
        p0 = normToAbs(pts[0], frame.shape)
