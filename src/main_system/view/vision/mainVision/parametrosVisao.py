...
from main_system.controller.tools.pixel2metric import  meters2pixel,meters2pixelSize
from main_system.view.vision.mainVision.visaoAltoNivel import VisaoAltoNivel
import cv2
import numpy as np
...
class ParametrosVisao(FrameRenderer):
  ...
  def ui(self):
    ...
    builder.get_object("use_clahe_switch").set_active(self.__visionSystem.use_clahe)
    builder.get_object("use_clahe_switch").connect("state-set", self.update_use_clahe)
    
    for i in range(5):
        builder.get_object(f"adj_robot_angle_{i}").set_value(self.__visionSystem.robot_angle_offsets[i])
        # Usamos default argument no lambda para fixar o valor de i
        builder.get_object(f"adj_robot_angle_{i}").connect("value-changed", lambda w, idx=i: self.update_robot_angle_offset(w, idx))

    self.__elements = [
        builder.get_object("adj_area_cont_rect"),
        builder.get_object("adj_min_area_internal_contour"),
        builder.get_object("adj_min_area_external_contour"),
        builder.get_object("use_clahe_switch"),
        builder.get_object("adj_robot_angle_0"),
        builder.get_object("adj_robot_angle_1"),
        builder.get_object("adj_robot_angle_2"),
        builder.get_object("adj_robot_angle_3"),
        builder.get_object("adj_robot_angle_4")
    ]
    
    self.__initial_values = [
        self.__visionSystem.areaRatio,
        self.__visionSystem.minInternalAreaContour,
        self.__visionSystem.minExternalAreaContour,
        self.__visionSystem.use_clahe,
        self.__visionSystem.robot_angle_offsets[0],
        self.__visionSystem.robot_angle_offsets[1],
        self.__visionSystem.robot_angle_offsets[2],
        self.__visionSystem.robot_angle_offsets[3],
        self.__visionSystem.robot_angle_offsets[4]
    ]
  ...
  def reset(self):
    for idx, el in enumerate(self.__elements):
      if isinstance(el, Gtk.Switch):
        el.set_active(self.__initial_values[idx])
      else:
        el.set_value(self.__initial_values[idx])
  ...
  def update_use_clahe(self, widget, state):
    self.__visionSystem.use_clahe = state
    
  def update_robot_angle_offset(self, widget, index):
    self.__controller.addEvent(self.__visionSystem.atualizarRobotAngleOffset, widget.get_value(), index)
  ...
  def getFrame(self):
    ...
    for i,allyPose in enumerate(message.allyPoses):
      if(allyPose[3]):
        ...
        Drawing.draw_rectangle(img_filtered, (x,y), (w,h), allyPose[2], color=(0,255,0), baseAngle=message.debug_baseAngles[i])
