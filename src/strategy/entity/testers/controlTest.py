...
# Inicia o arquivo de log de trajetória
...
class ControlTester(Entity):
    ...
    def fieldDecider(self):
        ...
        # Salva a posição exata para traçarmos o gráfico (usando tempo relativo)
        ...
        #Quad 4: X = -0.375 Y = -0.430        #Andar para frente e para trás
        if self.x == 1:
            if not -0.365 > rr[0] > -0.385 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = DirectionalField(ang(rr,[-0.375,0.430]), Pb=(-0.375,0.430, 0))
                
            else: self.x = 2
        if self.x == 2:
            if not 0.385 > rr[0] > 0.365 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = DirectionalField(ang(rr,[0.375,0.430]), Pb=(0.375,0.430, np.pi/2))
                
            else: self.x = 3
        if self.x == 3:
            if not 0.385 > rr[0] > 0.365 or not -0.420 > rr[1] > -0.440: #Não chegou no lugar certo
                self.robot.field = DirectionalField(ang(rr,[0.375,-0.430]), Pb=(0.375,-0.430, 0))
                
            else: self.x = 4
        if self.x == 4:
            if not -0.365 > rr[0] > -0.385 or not -0.420 > rr[1] > -0.440: #Não chegou no lugar certo
                self.robot.field = DirectionalField(ang(rr,[-0.375,-0.430]), Pb=(-0.375,-0.430, np.pi/2))
                
            else: self.x = 5
        if self.x == 5:
            if not -0.365 > rr[0] > -0.385 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = AttractiveField(Pb=(-0.375,0.430,0))
                
            else: self.x = 6
        if self.x == 6:
            if not 0.385 > rr[0] > 0.365 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = AttractiveField(Pb=(0.375,0.430,np.pi/2))
        ...
        if self.x == 9:
            if not -0.365 > rr[0] > -0.385 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = UVF(self.world, (-0.375,0.430, 0), robot=self.robot, direction=-1) 
                
            else: self.x = 10
        if self.x == 10:
            if not 0.385 > rr[0] > 0.365 or not 0.440 > rr[1] > 0.420: #Não chegou no lugar certo
                self.robot.field = UVF(self.world, (0.375,0.430, np.pi), robot= self.robot, direction=-1) 
